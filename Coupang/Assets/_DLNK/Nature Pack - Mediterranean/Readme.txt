[Mediterranean Nature Pack] version 1.1
Daelonik Artworks (www.dlnkworks.com)
...
[TEXTURES]
All textures in dlnk library are tileable & 2k / 4k sized. Also materials are customizable. Every material includes several maps for better pbr configuration.

[DEMOSCENE]
Demoscenes included (the ones used for advertising) where almost all prefabs are used. Chech it out to get an idea on how to properly use the different elements in a scene.
KEYS: [F5] Camera Switch [W][S] Movement [Mouse rotation] Camera [Space] Jump
** Scripts are included only for demo purposes

[GAIA BIOME]
Assets are compatible with Gaia & Unity built-in terrains. Gaia biome is included in a rar file. Extract it before use. (Gaia is required)

[RENDER PIPELINE]
Asset is built under Builtin Standard but can be switched to others. Custom shaders won't convert properly so ensure to switch all materials to Standard (except particles) before conversion or change them manually once converted. Special effects like leaves shader animation or water effects will be removed.

For extended info check out this link: https://www.dlnkworks.com/asset-store/environments/mediterranean-nature